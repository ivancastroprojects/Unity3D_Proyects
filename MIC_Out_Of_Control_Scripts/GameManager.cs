...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        endTimeText = GameObject.Find("/Canvas/EndGameText");
    ...
    public void loadElements()
    {
        ...
        //Debug.Log("barra" + barraVida.fillAmount);
        //Debug.Log("converted" + Health_and_Damage.converted);
        ...
        /** MISSION 2 **/
        if (GetToTextQuest() == "Horror in build MIC" && !onlyOneTimeAdvise)
        {
        ...
        if (SceneManager.GetActiveScene().name == "EXTERIORCASIFINAL" && (GetToTextQuest() == "Horror in build MIC" || GetToTextQuest() == "Without light and without defenses"))
        {
        ...
        if (quest.title == "Without light and without defenses" && quest.goal.currentAmount == 0)
        {
        ...
        else if (quest.title == "Without light and without defenses" && quest.goal.currentAmount == 1)
        {
            descriptionBoxExtra1.text = "1/1   Activates the electrical current of the MIC";
            lights.SetActive(true);
        }
        else if (quest.title == "Without light and without defenses" && quest.goal.currentAmount >= 2)
        {
            descriptionBoxExtra1.enabled = false;
            descriptionBoxExtra2.enabled = false;
            descriptionBox.text = "Completed. Go back to the Guard Scientist";
            lights.SetActive(true);
        }
        ...
        if (quest.activateSyringe)
        {
            ...
            if (SceneManager.GetActiveScene().name == "EXTERIORCASIFINAL" && !onlyOneTimeRememberAdds)
            {
                ...
                descriptionBoxExtra1.text = zombiesCountNormal + "/10   Mutants healed";
                descriptionBoxExtra2.text = zombiesCountBoss + "/1   Fifth patient healed";
            }
        ...
        if (quest.title == "Hero without cap" && !onlyOneTimeLoadScene)
        {
    ...
    public void lifeAgain()
    {
        ...
        if (SceneManager.GetActiveScene().name != "SegundaPlanta")
        {
            ...
            StartCoroutine(LoadAsynchronously("SegundaPlanta", "life"));
        }
        else
        {
    ...
    public void timeAgain()
    {
        ...
        {
            Debug.Log("NO estamos en la segunda"); 
            StartCoroutine(LoadAsynchronously("SegundaPlanta", "time"));
        }
        else
        {
            Debug.Log("SI estamos en la segunda");
    ...
    IEnumerator LoadAsynchronously(String sceneToLoad, String howEnded)
    {
