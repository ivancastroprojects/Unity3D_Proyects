...
public class DialogueTrigger : MonoBehaviour
{
    ...
    private void Update()
    {
        if(player == null)
            player = GameObject.FindGameObjectWithTag("Player");
        else dist = Vector3.Distance(player.transform.position, transform.position);
    ...
    public void TriggerDialogue()
    {
        ...
        if (GameManager.manager.GetToTextQuest() == "Hero without cap" && gameObject.name == "Director")
        {
        ...
        if (GameManager.manager.GetToTextQuest() == "Without light and without defenses" && GameManager.manager.quest.complete)
        {
            Debug.Log("Misión 4 completada");
            NPCcam.GetComponentInParent<DialogueManager>().StartDialogue(dialogue2, gameObject, NPCcam);
        } 
        else if (GameManager.manager.GetToTextQuest() == "End the mutation" && GameManager.manager.quest.complete)
        {
